...
#define READ_ADDR_SET_REG              UINT64_C(0x508)
#define WRITE_ADDR_SET_REG             UINT64_C(0x50C)
#define SET_A_LENGTH_REG               UINT64_C(0x510)
#define SET_B_LENGTH_REG               UINT64_C(0x514)
#define SET_C_LENGTH_REG               UINT64_C(0x518)
#define SET_A_WIDTH_REG                UINT64_C(0x51C)
#define SET_B_WIDTH_REG                UINT64_C(0x520)
#define SET_C_WIDTH_REG                UINT64_C(0x524)
#define FUNC_START                     UINT64_C(0x528)
#define WORK_DONE                      UINT64_C(0x52C)
#define baseaddr_A                     UINT64_C(0x000)
#define baseaddr_B                     UINT64_C(0x000)
#define baseaddr_C                     UINT64_C(0x040)
...
int peek_poke_example(uint32_t value, int slot_id, int pf_id, int bar_id) {
    ...
    uint32_t i, value;
    uint32_t Vector_Size = 64;
    uint32_t matrix_size = 8;
    srand(3);
    uint32_t matrix_A[Vector_Size];
    uint32_t matrix_B[Vector_Size];
    uint32_t matrix_C[Vector_Size];
    // Initialize the vector A and Vector B
    for (i = 0; i < Vector_Size; i++){
        matrix_A[i] = (i + 1) % 4;
        printf("0x%x", matrix_A[i]); 
   }
    for (i = 0; i < Vector_Size; i++){
        matrix_B[i] = (i + 1) % 4;
        printf("0x%x", matrix_B[i]); 
   }
    rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, Vector_Size);
    fail_on(rc, out, "Unable to write to the fpga !");      
    uint32_t state = 0;
                 
    rc = fpga_pci_poke(pci_bar_handle, SET_A_LENGTH_REG, matrix_size);
    fail_on(rc, out, "Unable to write to the fpga !");
    rc = fpga_pci_peek(pci_bar_handle, SET_A_LENGTH_REG, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);

    rc = fpga_pci_poke(pci_bar_handle, SET_A_WIDTH_REG, matrix_size);
    fail_on(rc, out, "Unable to write to the fpga !");
    rc = fpga_pci_peek(pci_bar_handle, SET_A_WIDTH_REG, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);
    
    rc = fpga_pci_poke(pci_bar_handle, WRITE_ADDR_SET_REG, baseaddr_A);
    fail_on(rc, out, "Unable to write to the fpga !");
    rc = fpga_pci_peek(pci_bar_handle, WRITE_ADDR_SET_REG, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);
    
    
    for (i = 0; i < Vector_Size; i++){
        rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, matrix_A[i]);
        fail_on(rc, out, "Unable to write to the fpga !");
        printf("write %x to data BRAM  \n", i); 
   }
   
    rc = fpga_pci_poke(pci_bar_handle, READ_ADDR_SET_REG, baseaddr_A);
    fail_on(rc, out, "Unable to write to the fpga !");

    for (i = 0; i < Vector_Size; i++){
    rc = fpga_pci_peek(pci_bar_handle, HELLO_WORLD_REG_ADDR, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);
   }    
      
    /* read Vec_A back and print it out; you should expect the byte order to be
     * reversed (That's what this CL does) */
    rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, Vector_Size);
    fail_on(rc, out, "Unable to write to the fpga !");      
    uint32_t state = 0;
                 
    rc = fpga_pci_poke(pci_bar_handle, SET_B_LENGTH_REG, matrix_size);
    fail_on(rc, out, "Unable to write to the fpga !");
    rc = fpga_pci_peek(pci_bar_handle, SET_B_LENGTH_REG, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);

    rc = fpga_pci_poke(pci_bar_handle, SET_B_WIDTH_REG, matrix_size);
    fail_on(rc, out, "Unable to write to the fpga !");
    rc = fpga_pci_peek(pci_bar_handle, SET_B_WIDTH_REG, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);
    
    rc = fpga_pci_poke(pci_bar_handle, WRITE_ADDR_SET_REG, baseaddr_B);
    fail_on(rc, out, "Unable to write to the fpga !");
    rc = fpga_pci_peek(pci_bar_handle, WRITE_ADDR_SET_REG, &state);
    fail_on(rc, out, "Unable to read from fpga");
    printf("status: 0x%x\n", state);
    ...
    for (i = 0; i < Vector_Size; i++){
        rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, matrix_B[i]);
        fail_on(rc, out, "Unable to write to the fpga !");
        printf("write %x to data BRAM  \n", i); 
   }
   
    rc = fpga_pci_poke(pci_bar_handle, READ_ADDR_SET_REG, baseaddr_B);
    fail_on(rc, out, "Unable to write to the fpga !");
